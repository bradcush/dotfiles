...
beautiful.get().wallpaper =
    '/home/bradcush/Downloads/pexels-anni-roenkae-3418396.jpg'

-- This is used later as the default
-- terminal and editor to run.
local terminal = 'alacritty'
local editor = os.getenv('EDITOR') or 'vim'
local editor_cmd = terminal .. ' -e ' .. editor

-- Default modkey, usually Mod4 is the key
-- with a logo between Control and Alt
local modkey = 'Mod4'

-- Table of layouts to cover with awful
-- layout.inc, order matters.
...
local mylauncher = awful.widget.launcher({
    -- Hide icon which can trigger menu
    -- image = beautiful.awesome_icon,
    menu = mymainmenu
})

-- Menubar config which sets the terminal
...
-- Set the wallpaper if we have some
local function set_wallpaper(s)
    -- When we have Wallpaper
...
-- Re-set wallpaper when a screen's geometry
-- changes like for different resolutio
screen.connect_signal('property::geometry', set_wallpaper)

-- Connect things for each screen
...
-- LuaFormatter off
-- Binding actions to mouse activity
local toggle_menu = function() mymainmenu:toggle() end
...
-- make it work on any keyboard layout. This should map on the
-- top row of your keyboard, usually 1 to 9.
for i = 1, 9 do
    ...
    }, -- Floating clients.
    {
        rule_any = {
            ...
            class = {
                ...
                'MessageWin', -- kalarm.
                'Sxiv',
                -- Needs a fixed window size to avoid
            ...
            role = {
                ...
                'pop-up' -- Like Google Chrome's (detached) Developer Tools
            }
        },
...
-- Signal execution when a new client appears
client.connect_signal('manage', function(c)
    -- Set the windows at the slave, i.e. put it at the
    -- end of others instead of setting it master
    if awesome.startup and not c.size_hints.user_position and
        not c.size_hints.program_position then
        -- Prevent clients from being unreachable
        -- after the screen count changes
